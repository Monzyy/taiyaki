...
class Model(Module):
    ...
    """
    def __init__(self, config, flipflop=False):
        super(Model, self).__init__()
        self.stride = config['block'][0]['stride'][0]
        self.alphabet = config['labels']['labels']
        self.features = config['block'][-1]['filters']
        self.encoder = Encoder(config)
        # Atm labels contain all flips and flops, maybe change this to just the nucleotides.
        self.decoder = Decoder(self.features, flipflopfings.nstate_flipflop(len(self.alphabet)))
        self.sublayers = nn.ModuleList((self.encoder, self.decoder))
...
class Encoder(Module):
    ...
    def __init__(self, config):
        ...
        self.sublayers = nn.ModuleList(encoder_layers)
...
class TCSConv1d(Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False, separable=False):
        ...
        if separable:
            ...
            self.sublayers = nn.ModuleList((self.depthwise, self.pointwise))
        else:
            self.conv = Conv1d(
                in_channels, out_channels, kernel_size=kernel_size,
                stride=stride, padding=padding, dilation=dilation, bias=bias
            )
            self.sublayers = nn.ModuleList((self.conv, ))
...
class Decoder(Module):
    ...
    def __init__(self, features, classes):
        ...
        self.nbase = flipflopfings.nbase_flipflop(classes)
