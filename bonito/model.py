...
class Model(Module):
    ...
    def __init__(self, config):
        ...
        self.decoder = Decoder(self.features, flipflopfings.nstate_flipflop(len(self.alphabet) - 1))
...
class Block(Module):
    ...
    def forward(self, x):
        ...
        for layer in self.conv:
            _x = layer(_x)
        if self.use_res:
            _x += self.residual(x_0)
        #_x = _x.permute(0, 2, 1)
        return [self.activation(_x).permute(0, 2, 1)]
...
class Decoder(Module):
    ...
    def forward(self, x):
        x = self.layers(x[-1].permute(0, 2, 1))
